...
parser.add_argument('--anoscore', type = str, help = 'trendfilter, diff, STL or STL_resid',default = 'trendfilter')
parser.add_argument('--score_func', type = str, help = 'exp,dirac',default = 'dirac')
...
parser.add_argument('--lay_back', type=int, default=1, help='lay back in calculating anoscore')
...
if args.ano_type == 'gaussian':
    args.ano_scale = 2.0
#save_checkpoint_dir = 'results/ETT/base_lstm'
#save_checkpoint_dir = 'results/base_lstm'

args.this_method = 'base_lstm_weighting'
...
def cal_anoscore(src,func = 'dirac',start_T = 15,end_T = 16):
    anoscore = 0
    if func == 'exp':
        for t in range(start_T,end_T):
            anoscore += np.exp(-np.square(t-(end_T-1) ) )*src[t]
    elif func=='dirac':
        for t in range(start_T,end_T):
            anoscore += src[t]
    return anoscore
def diff_series(src,win_size = 1):
    src = src.data.cpu().numpy()
    batch_size,length = src.shape
    length_new = length - win_size
    src_new = np.zeros((batch_size,length_new))
    for i in range(batch_size):
        for j in range(length_new):
            src_new[i,j] = src[i,j+win_size] - np.mean(src[i,j:j+win_size])
    return np.abs(src_new)
...
if __name__ == '__main__':
    for epoch in range(args.epochs):
        ...
        for ii, (index,samples, samples_trend,samples_seasonal,samples_resid, labels) in enumerate(train_loader):
            ...
            samples = samples.reshape(samples.shape[0],samples.shape[1],1)
            samples = samples.to(torch.float32).to(args.device)
            samples_trend = samples_trend.to(torch.float32).to(args.device)
            samples_seasonal = samples_seasonal.to(torch.float32).to(args.device)
            samples_resid = samples_resid.to(torch.float32).to(args.device)
            labels = labels.to(torch.float32).to(args.device)
            hidden = torch.zeros(args.lstm_layers, batch_size, args.hidden_size).to(args.device)
            cell = torch.zeros(args.lstm_layers, batch_size, args.hidden_size).to(args.device)
            points,hidden,cell = model(samples,hidden,cell) 
            ...
            if args.anoscore == 'trendfilter':
                samples_squeeze = torch.squeeze(samples)
                err = torch.abs(samples_squeeze-samples_trend)
            elif args.anoscore == 'diff':
                err = diff_series(torch.squeeze(samples),win_size = args.win_size)
            elif args.anoscore == 'STL':
                samples_squeeze = torch.squeeze(samples)
                err = torch.abs(samples_squeeze-samples_trend)
            elif args.anoscore == 'STL_resid':
                err = torch.abs(samples_resid)
            end_T = err.shape[1]
            start_T = end_T - args.lay_back
            loss_v = torch.zeros(batch_size)
            for i in range(batch_size):
                pscore = cal_anoscore(err[i],func = args.score_func,start_T=start_T,end_T=end_T) 
                if pscore<= args.thre:
                    loss_v[i] = 1.0
            cal_eff_samples[0]+=1
            cal_eff_samples[1]+=sum(loss_v)
            loss = torch.mean(loss_v * loss_batch)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        print('batch size of effective samples', float(cal_eff_samples[1])/cal_eff_samples[0])
